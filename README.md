...
⚠️ Disclaimer: This project was developed as a personal security tool. While it implements standard cryptographic functions (Argon2id, AES-GCM), Always evaluate your own security needs before using my personal project for sensitive data.

## Features
- Argon2id to derive 256-bit encryption keys.
- AES-GCM encryption
- Support for piping into other CLI tools.

## How To Use
On first use, PW Vault initialises an encrypted vault file in a standard application data directory upon the first execution of any protected command.
...
If you wish for a complex password to be generated automatically, use the -g flag. 

    ./pw-vault create github -u my_username -e user@example.com -g

If you use the same username and / or email address for multiple serves, a preset can be setup, allowing quick account creation.
...
You can get account data by specifing the service.
    ...
    ./pw-vault get github
    Enter Your Password: 
    [PWVAULT]: Service: github
    [PWVAULT]: User:    my_username
    [PWVAULT]: Email:   user@example.com
    [PWVAULT]: Password: =A**********M<<
